...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 z-[1000] w-full border-b border-accent/20 bg-secondary/80 shadow-[0_14px_34px_rgba(20,18,16,0.08)] backdrop-blur-lg">
      <div className="mx-auto flex h-[106px] max-w-[1400px] items-center justify-between gap-6 px-6 md:px-12">
      ...
      {open && (
        <div className="border-t border-border bg-secondary/95 lg:hidden">
          <nav className="mx-auto grid max-w-[1400px] gap-1 px-6 py-6 md:px-12">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                onClick={() => setOpen(false)}
                className="py-3 text-sm font-bold uppercase tracking-[0.14em] text-primary"
              >
                {link.name}
              </Link>
            ))}
